...
static struct argp_option options[] = {
    ...
    {"plantowatch", OPT_PLANTOWATCH, 0, 0,
     "Fetch a user's plan to watch anime"},
...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
    ...
    switch (key) {
        ...
        case OPT_WATCHING:
            ...
            break;
        case OPT_COMPLETED:
            arguments->mode = COMPLETED_MODE;
            break;
        case OPT_ONHOLD:
            arguments->mode = HOLD_MODE;
            break;
        case OPT_DROPPED:
            arguments->mode = DROPPED_MODE;
            break;
        case OPT_PLANTOWATCH:
            arguments->mode = PLAN_MODE;
            break;
        case OPT_ALL:
 ...
 * -----------------------
 * Curl callback which assigns the delivered data to a properly sized struct
 * buffer
 *
 ...
 */
CURLcode curl_fetch_url(CURL *curl, const char *url,
 ...
 */
void generate_anime_api_uri(char *uri, size_t uri_size, char *username,
                            char *endpoint, int allow_nsfw) {
 ...
 */
void print_anime_list(struct json_object *anime_list, size_t page,
                      char *list_name) {
...
int main(int argc, char *argv[]) {
    ...
    generate_anime_api_uri(uri, uri_size, arguments.args[0], endpoint,
                           arguments.nsfw);
